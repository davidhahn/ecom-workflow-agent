...
- **Components:** Hero (eyebrow, `h1`, two-sentence pitch, three capability badges, a primary CTA into `/scenarios`, a GitHub link, then a stat strip of three pills linking to Evaluation Lab, Architecture, and Activity). `SnapshotTabs` comes next: three tabs, injection attempt, data analysis, refund decision. Each one shows a captured fixture, with its own "Run this scenario live" link into `/scenarios`. A "How it works" section follows, three short steps plus the architecture diagram. Then "What the evals actually show": four hand-picked eval-category stat cards, two finding cards linking into the Evaluation Lab's findings report, and a line naming whichever categories are still short of 100%. An "Explore" grid closes the page, one card per other page, each with that page's job in a sentence.
- **Data:** `getEvalResults()` and `RESPONSIBILITY_ROWS` (`lib/architecture.ts`), plus the captured scenario results in `lib/scenario-snapshots.json`, read through `lib/snapshots.ts`. Same static, build-time sources the old page used, plus one more. No number here gets typed in by hand.
- **States:** None. The page renders statically on the server, under the same fail-loud rule as Evaluation Lab and Architecture. A missing eval artifact breaks the build. No wrong number ever reaches the page.
...
- **Components:** A static, hand-curated `SCENARIOS` array. Its data now lives in `lib/scenarios.json`, so a Python capture script can read the exact same inputs `scenarios.ts` types against. Still no authoring UI. Five `ScenarioCard`s (data analysis, policy retrieval, refund approval, ambiguous refund refusal, injection attempt). Each one shows a name, a one-sentence business context, and an "expected behavior" statement collapsed behind a native `<details>`. That details block opens by default now, since a result sits on screen the moment the card loads. Right below it sits a captured snapshot, with a "Captured from a real run on {date}" caption. A "Run it fresh" button replaces that snapshot with a live result and a working trace link. Below the cards, collapsed behind its own `<details>`, sits a free-form refund request box, folded in from the former standalone Refunds page, for a viewer who wants to try their own input against the same deterministic gate. A `NextSteps` block sits at the bottom, pointing to Ask and Activity.
- **Data:** A live run returns `AnalyzeResponse` from `POST /api/query/analyze` (data-analysis and policy-retrieval cards) or `RefundEvaluateResponse` from `POST /api/refund/evaluate` (the other three cards, plus the free-form box). The snapshot shown before that comes from `lib/scenario-snapshots.json`. `evals/capture_scenario_snapshots.py` captured it against the real endpoints and committed it. Nobody typed it by hand. Result rendering is shared via `AnalyzeResult`/`RefundResult` components, also reused by `/ask`, the free-form box, and the landing page's `SnapshotTabs`.
- **States:** Snapshot: the card shows its captured result right away. No trace link appears, since a captured `request_log_id` may point at a row the demo DB has already reset. Loading: button reads "Running…", disabled. Error: red panel with message; 429 shows retry-after. Success: badge row + answer (analyze) or status badge + rule + reasoning + extracted fields (refund), plus a "View execution trace" link.
- **Navigation:** Each live result's trace link goes to `/activity/{request_log_id}`. A `NextSteps` block at the bottom points to Ask and Activity.
...
- **Components:** Textarea and submit button first. An `ExampleChip` row sits right below, visible immediately, no `<details>` toggle hiding it anymore. An empty textarea used to be the whole first impression here. `AnalyzeResult` handles the answer (badge row, warning panels, `Markdown` answer, sources, trace link). A `NextSteps` block sits at the bottom, pointing to Activity and Evaluation Lab.
...
- **Navigation:** New "Evaluation Lab" tab in `NavHeader`. A `NextSteps` block at the bottom, pointing to Architecture and GitHub.
...
- **Navigation:** A new "Architecture" tab in `NavHeader`, linking to the relevant `DECISIONS.md` entries for Part 4. A `NextSteps` block at the bottom, pointing back to Scenarios and to the full `DECISIONS.md` on GitHub.

## Design constraints

One accent color sits on the existing gray and border palette now: `--accent` and `--accent-foreground` in `globals.css`, teal in light mode and a lighter teal in dark mode. It marks primary buttons and active nav state. It shows up on key stat numbers and text links too. That's the accent's whole job. Page content sits inside a `max-w-5xl` container now. The original was `max-w-4xl`. `NavHeader` is sticky now, with a blurred translucent background behind it, so it stays readable over content scrolling underneath. `SiteFooter` renders on every page below `<main>`. This app never had a real footer before.

`Button`, `Badge`, `Card`, and `Tabs` aren't hand-rolled anymore. `components/ui/` holds the real source for each, pulled from shadcn/ui and beUI. Any adaptation is disclosed right in that file's own top comment, things like `Card`'s radius or a swapped hover color, not silent changes. `NavHeader`'s active-link underline reuses `Tabs`'s spring indicator technique, though it isn't a beUI file itself. There's no separate beUI nav component, so this reapplies the same pattern to real page navigation instead of client-side tab state. `components.json` documents the setup. `DECISIONS.md` #60 has the full account: what changed, the two new runtime dependencies, and the token mapping that makes these components read this app's teal system instead of shadcn's own default theme.

Reuse `Badge`, `Button`, `Card`, `Tabs`, `ToolCallTrace`, `Markdown`, `ExampleChip`, `JsonPreview`, `NextSteps`, and the newer `AnalyzeResult`, `RefundResult`, `ScenarioCard`, `SnapshotTabs`.

There's no role picker anywhere in the UI. The role stays pinned to `read_only_viewer` through `RoleProvider`, and `setRole` is never called. That's fine for now. Neither `/query/analyze` nor `/refund/evaluate` checks the role at all. The tool-backed endpoints that do check it, tickets, invoices, direct SQL, aren't wired into any page yet. A role picker would be UI with nothing behind it. Build one once a page calls a role-gated endpoint. All views keep the strict loading, error, and empty handling above. Nothing here uses a silent fallback.

## Explicitly not building

Admin dashboard. Settings page. Advanced filtering or search on Activity. Custom design system or theme redesign. General animation work, beyond the sourced spring indicators already on `Tabs` and `NavHeader`. New analytics infrastructure. A scenario-authoring UI (the curated set is a static array, edited by hand). Additional backend capabilities beyond the one `request_log_id` field the trace link needed.
