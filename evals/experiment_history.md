...
What changed, what the measurement showed, and what decision followed. Rows marked **MEASUREMENT CHANGE** altered the ruler itself. Read a score move on those rows as evidence about the ruler, separate from the product.
...
| Add semantic SQL assertions (check the returned value, not just query shape) | Exposed 14/21 (67%): legal SQL computing the wrong number, invisible to shape-only checks | fix eval, then investigate | **MEASUREMENT CHANGE** |
...
| Groundedness matches rule titles, not just numbers | `ground-01` flips from a false ungrounded flag to correctly grounded | keep | |
...
**Widening RAG's `k`** looked like a plausible fix for a known retrieval gap. It made things worse somewhere else. Off-topic questions started leaking through the same door meant to let more of the right chunk in.

**The Haiku swap** held its own on most categories. It lost ground on the two that carry financial and compliance weight. Sonnet stays the deployed model.

## mixed-07: a new, open finding

Part 2's live rerun caught something no saved report had. `mixed-08` is fixed and stayed fixed. `mixed-07` failed on a problem nobody had written a case for. The judge caught the answer disputing a stated fact it should have confirmed against the real data. That row stays open, a candidate for the next round of this table.
