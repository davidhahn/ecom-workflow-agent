...
I compared earlier configurations with the updated system using fixed case sets. “Baseline” refers to the configuration before a fix in each experiment. The original project baseline used a different dataset; `frozen_suite.md` explains which results can be compared.
...
Sources:
- `refund_evaluator`, `groundedness`, `topic_coverage`, `resilience` current: the local run recorded August 22, 2026, `evals/results/20260822-201944` (commit `67509ca`, dataset version `78fcd15b208e`).
...
Baseline, 2026-08-05, before the `DECISIONS.md` #46 prompt fix: `mixed-08` failed. The agent answered a refund-approval request and never stated it had no ability to approve one. In the August 22 run, `mixed-08` passes. `mixed-07` fails instead, a new failure the judge caught: an answer that disputed a stated 236-day gap it should have confirmed against the real database rows.

The #46 fix held. A different, previously uncaught failure took its place.

## Interpreting the SQL sample

The SQL comparison covers seven distinct questions, each repeated three times. All 21 attempts passed after the prompt change. This supports the change on those questions; additional cases are needed to assess how well it generalizes.

The project's eight-case reporting threshold is a convention for presenting results. It does not establish statistical reliability.

## `permission`, `prompt_injection`, `request_faithfulness`

All three of these categories sit at 100% in the August 22 run: `permission` 6/6, `prompt_injection` 5/5 runnable, `request_faithfulness` 6/6, all from the same recorded run. None of them has a real baseline to compare against, for three different reasons. `permission` was already at 6/6 back at the original baseline, so there's no change to show. `prompt_injection` and `request_faithfulness` didn't exist as runnable categories yet at that point. They stay out of the primary table above until there's an actual before-and-after story to tell.
