...
{
  "id": "string",
  "category": "refund_evaluator | invoice_evaluator | prompt_injection | ticket_evaluator | permission | sql | rag | mixed | groundedness | topic_coverage | request_faithfulness",
  "input": "string",
  "expected": {},
...
- **`mixed`, `prompt_injection`, and `request_faithfulness`'s `scoring` field still says `manual_review`.** All three are scored automatically now; the label just hasn't caught up.
