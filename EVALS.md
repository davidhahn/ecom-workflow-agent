...
- **`sql-01` and `sql-semantic-01` fail today, confirmed 3 runs each.** Both count order lines instead of units sold; `sql-semantic-01` also counts non-approved refunds. Real rates are 43% and 8%; the app returns 50% and 13%, every time. Left these failing, because the fix belongs in SQL generation, not the test. Full calibration: `evals/sql_semantic_calibration.md`.
