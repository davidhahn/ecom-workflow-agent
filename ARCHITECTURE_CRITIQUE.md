...
Each finding below now carries a status, added after this review ran its course. Fixed means the code changed and I checked the fix held up. Accepted means I looked hard at the risk and chose to leave it, usually because Part 1's scope has no better answer yet. Open means nothing has changed since the finding was written. Of the ten findings here, seven got fixed and two got accepted. One is still open.
...
**Status: Accepted.** No authentication exists anywhere in the API. It's a documented non-goal, in `PRODUCT_SPEC.md` and in this README's own Limitations section. Finding #2's fix narrows the compounded risk this finding described. A wrong-customer decision can no longer happen silently. The underlying gap hasn't moved. Real deployment needs a real identity provider first.
